{
  ...
  for(int i=0;i<nm;i++) {
    obs_mm[i] /= (xsec_alp[i]*k_factor);
    obs_em[i] /= (xsec_alp[i]*k_factor*2.);
    obs_ee[i] /= (xsec_alp[i]*k_factor);
 ...
 // CMS_lumi( c1, 2, 0 );
  ...
  leg->AddEntry(graphs[1], "Observed  #||{V_{eN}}^{2}", "l");
  leg->AddEntry(graphs[2], "Observed  #frac{#||{V_{eN}V*_{#muN}}^{2}}{#||{V_{eN}}^{2} + #||{V_{#muN}}^{2}}", "l");
  leg->AddEntry(graphs[0], "Observed  #||{V_{#muN}}^{2}", "l");
 ...
 // c1->GetFrame()->Draw();
  ...
  c1->SaveAs("excl_limit_combined.pdf");
}
