{
 ...
 // CMS_lumi( c1, 2, 0 );
  ...
  frame->GetYaxis()->SetTitle("#sigma #times Br(pp #rightarrow N  l_{1}^{#pm}/l_{2}^{#pm} #rightarrow l_{1}^{#pm}l_{2}^{#pm} q#bar{q'}) [pb]");
  ...
  leg->AddEntry(graphs[0], "CL_{s} Expected  #mu#mu", "l");
  leg->AddEntry(graphs[1], "CL_{s} Expected ee", "l");
  leg->AddEntry(graphs[2], "CL_{s} Expected  #mue", "l");
 ...
 // c1->GetFrame()->Draw();
  ...
  c1->SaveAs("excl_xsec_explimit_combined.pdf");
}
