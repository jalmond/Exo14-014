{
 ...
 // CMS_lumi( c1, 2, 0 );
  ...
  frame->GetYaxis()->SetTitle("#sigma #times Br(pp #rightarrow #mu^{#pm}N  #rightarrow  #mu^{#pm}e^{#pm} q#bar{q'}) [pb]");


  frame->GetYaxis()->SetTitleSize(0.05);
  //frame->GetYaxis()->SetTitleOffset(0.95);
  //frame->GetYaxis()->SetLabelSize(0.04);
  frame->GetYaxis()->SetRangeUser(0.0001,1.0);
  frame->Draw(" ");
  c1->SetLogy();
 ...
 // c1->GetFrame()->Draw();
  ...
  c1->SaveAs("excl_limit_xsec_mm.pdf");
}
